...
#NOTE: We will need to think about data purging (when do we remove handouts and invites etc)
...
def login(request):
	...
	try:
		...
		#EXPAND HERE
		res = {'errcode':valid,'user_id':errcode[0],'invites':0, 'teacher':errcode[2], 'period':errcode[3]}
...
def add_user(request):
	...
	errcode = db_model.add_user(username,school,teacher,period,first_name,last_name,email)
...
def get_classmates(request):
	...
	try:
		...
		if result[2]==-1:
			res = {'errcode':1,'first_name':None,'last_name':None, 'user_id':None}
		else:
			res = {'errcode':1,'first_name':result[0],'last_name':result[1], 'user_id':result[2]}	
		return HttpResponse(json.dumps(res), content_type='application/json')
	except Exception, ex:
...
def send_invites(request):
	...
	try:
		...
		errorcode=1
		for x in u:
			error = db_model2.put_invite(x,h)
